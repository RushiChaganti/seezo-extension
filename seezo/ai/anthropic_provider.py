...
class AnthropicProvider(AIProvider):
    ...
    def __init__(self, model_name: str = "claude-3-opus-20240229", api_key: Optional[str] = None):
        ...
        logger.info(f"Initializing AnthropicProvider with model: {model_name}")
    ...
    async def analyze_security(self,
        ...
        try:
            logger.info(f"Sending request to Anthropic model: {self.model_name}")
            ...
            response = self._client.messages.create(
                ...
                messages=[
                    {"role": "system", "content": "You are a security expert analyzing web applications for vulnerabilities."},
            ...
            logger.error(f"Anthropic analysis failed: {str(e)}", exc_info=True)
            return SecurityAnalysisResult(
                vulnerabilities=[],
                sensitive_assets=[],
                recommendations=[],
